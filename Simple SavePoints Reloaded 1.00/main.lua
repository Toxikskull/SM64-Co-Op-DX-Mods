-- name: \\#00FF7F\\Simple SavePoints Reloaded v1.00D\\#00FF7F\\
...
local function refreshSlideFunc(m) --GHETTO SLIDE FIX CODE (HAS LOADING SAVEPOINT BUGS) (WORKS AFTER LOADING SAVE WHEN HITTING SURFACE_TIMER_END AFTER SERVER RELOAD) FUCKING FIX!!!!!
	...
	if refreshSlide > 1 then
		...
		if raceTimerValue <= 3 then raceTimerRunning = false end --STOP TIMER IF SPAWNING OUTSIDE THE SLIDE/RACE BUT FROM ALSO FROM THE LEVEL
		refreshSlide = 0
		end
	end

	if refreshSlide == 1 then djui_chat_message_create("WARPING BACK") warp_to_warpnode(s.spSLevel, s.spSArea, s.spSAct, WARP_NODE_DEATH) refreshSlide = 2 end
...
local function updateWarpedBowserReset()
    ...
    if o ~= nil and (m == nil or m.heldObj ~= o) and s.spObjMode ~= OBJ_BOWSER
        ...
        o.oAction = 5 --SET ACTION TO 5 SO BOWSER DOESN'T ATTACK DURING DIALOGUE (GHETTO FIX MAY FREEZE BOWSER WHILE ANOTHER PLAYER IS IN ARENA AND RELOADING SAVE)
    end
    bowserTime = 0
end
...
local function tracktime()
    ...
    djui_chat_message_create("Keep track of timers: " .. (trackTimer and "\\#6fd83f\\enabled\\#6fd83f\\" or "\\#dd3232\\disabled\\#dd3232\\"))
    return true
end
